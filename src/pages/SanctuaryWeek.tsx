...
const weekOne: CeremonyCardData[] = [
  ...
  },
  {
    ...
    description: "A full day of body treatments, sound ceremony, sacred nourishment, yoga, and ceremonial closing. From sunrise to sunset, every moment held.",
...
const weekTwo: CeremonyCardData[] = [
  ...
  },
  {
    ...
    tags: [{ label: "Purification" }, { label: "Application Required", type: "app" }, { label: "Sacred Screening" }],
    pricing: [
...
const CeremonyCard = ({ card }: { card: CeremonyCardData }) => {
  ...
  return (
    ...
    >
      <Link
        to={card.slug}
        className="group flex flex-col bg-[hsl(105,30%,12%)] border border-[hsl(100,25%,18%)] hover:border-[hsl(45,50%,40%)] transition-colors overflow-hidden"
      >
        <img
          src={card.image}
          alt={card.name}
          className="w-full h-[220px] object-cover saturate-[0.7] brightness-[0.8] group-hover:saturate-[0.9] group-hover:brightness-[0.9] transition-[filter] duration-400"
          loading="lazy"
        />
        <div className="p-6 md:p-7 flex-1 flex flex-col">
          ...
          <p className="font-serif italic text-[15px] text-[hsl(45,60%,55%)] mb-3.5">
            {card.subtitle}
          </p>
          <p className="text-[15px] text-[hsl(35,30%,68%)] leading-relaxed flex-1 mb-4 font-serif">
            {card.description}
          </p>
          ...
          <div className="flex gap-3 flex-wrap mb-4">
            {card.pricing.map((tier) => (
              <span
                key={tier.label}
                className="font-sans text-[8px] tracking-[1.5px] uppercase text-[hsl(35,20%,50%)]"
              >
                <span className="text-[hsl(45,70%,49%)]">{tier.price}</span>
                {" "}
                {tier.label}
              </span>
            ))}
            ...
            {card.tags.map((tag) => (
              <span
                ...
                className={`font-sans text-[7px] tracking-[2px] uppercase border px-2.5 py-1 ${
                  tag.type === "free"
                    ? "text-[hsl(110,40%,50%)] border-[hsl(110,30%,30%)]"
                    : tag.type === "app"
                    ? "text-[hsl(45,60%,55%)] border-[hsl(45,40%,30%)]"
                    : "text-[hsl(35,20%,50%)] border-[hsl(100,20%,22%)]"
                }`}
              >
                {tag.label}
              </span>
          ...
          {card.ctaReady ? (
            <a
              ...
              className="block text-center font-sans text-[8px] tracking-[2px] uppercase py-3 px-5 border border-[hsl(45,50%,35%)] text-[hsl(45,70%,49%)] hover:bg-[hsl(45,70%,49%)] hover:text-[hsl(105,30%,5%)] transition-all mt-auto"
            >
              {card.ctaLabel}
            </a>
          ) : (
            <span className="block text-center font-sans text-[8px] tracking-[2px] uppercase py-3 px-5 border border-[hsl(100,20%,22%)] text-[hsl(35,20%,42%)] mt-auto">
...
const SanctuaryWeek = () => (
  ...
  >
    ...
    <section className="relative min-h-screen flex flex-col justify-end px-6 md:px-12 py-16 md:py-20 overflow-hidden">
      ...
      >
        ...
        <div className="flex gap-4 flex-wrap">
          <a href="#ceremonies" className="inline-block font-sans text-[9px] tracking-[3px] uppercase px-10 py-4 bg-[hsl(45,70%,49%)] text-[hsl(105,30%,5%)] hover:bg-[hsl(45,70%,58%)] transition-all">
            See All Ceremonies
          </a>
          ...
          <Link to="/community-care" className="inline-block font-sans text-[9px] tracking-[3px] uppercase px-10 py-4 bg-transparent border border-[hsla(45,70%,49%,0.5)] text-[hsl(45,70%,55%)] hover:bg-[hsla(45,70%,49%,0.1)] transition-all">
    ...
    {(() => {
      ...
      return (
        <div className="bg-[hsla(45,70%,49%,0.08)] border-y border-[hsla(45,70%,49%,0.15)] py-6 px-6 md:px-12 text-center">
          <p className="font-sans text-[9px] tracking-[4px] uppercase text-[hsl(45,70%,49%)] mb-2">Monthly Theme</p>
          <p className="font-sans text-[clamp(16px,2vw,22px)] font-light text-[hsl(40,30%,90%)] mb-1">{current.theme}</p>
          <p className="font-serif italic text-[14px] text-[hsl(35,30%,60%)]">{current.desc}</p>
        </div>
      );
    })()}
    ...
    {/* ORIGIN STORY */}
    <SanctuarySection eyebrow="Year Six · Spring Equinox 2026" title={<>Six Years Ago<br /><em className="font-serif italic text-[hsl(45,70%,55%)] text-[1.1em]">We Didn't Know What We Were Building</em></>}>
      <div className="text-xl leading-[1.85] text-[hsl(35,30%,68%)] max-w-[720px] mx-auto text-center font-serif space-y-6">
        ...
        <p>That somewhere has grown into a sacred temple and 501(c)(3) nonprofit organization, a sacred ecosystem, and a family. This March is our Grand Reopening, and the beginning of our most powerful chapter yet.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-0.5 mt-16">
        {[
          ...
          <div key={item.label} className="bg-[hsl(105,30%,12%)] p-8 border border-[hsla(45,70%,49%,0.12)] text-center">
            <p className="font-sans text-3xl font-extralight text-[hsl(45,70%,49%)] mb-2">{item.stat}</p>
            <p className="font-sans text-[9px] tracking-[2px] uppercase text-[hsl(40,30%,90%)] mb-2">{item.label}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-0.5 mt-0.5">
        {[
          ...
          <div key={card.title} className="bg-[hsl(105,30%,12%)] p-8 md:p-9 border border-[hsla(45,70%,49%,0.12)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-0.5">
        {[
          ...
          <div key={pillar.num} className="bg-[hsl(105,30%,12%)] border border-[hsla(45,70%,49%,0.12)] p-7">
            <p className="font-sans text-2xl font-extralight text-[hsl(45,70%,49%)] mb-3">{pillar.num}</p>
    ...
    <SanctuarySection eyebrow="The Rhythm of the Temple" title={<>Why We Close<br /><em className="font-serif italic text-[hsl(45,70%,55%)] text-[1.1em]">After the Winter Solstice</em></>}>
      ...
      <blockquote className="mt-16 border-l-[3px] border-[hsl(45,70%,49%)] p-8 bg-[hsla(45,70%,49%,0.05)] max-w-[720px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-0.5">
        {[
          ...
          <div key={slot.time} className="bg-[hsl(105,30%,12%)] border border-[hsla(45,70%,49%,0.12)] p-7 flex gap-5">
            <p className="font-sans text-[10px] tracking-[2px] text-[hsl(45,70%,49%)] whitespace-nowrap pt-1">{slot.time}</p>
            <div>
    ...
    {/* THE ARC OF TEN DAYS */}
    <SanctuarySection eyebrow="The Journey" title={<>The Arc of<br /><em className="font-serif italic text-[hsl(45,70%,55%)] text-[1.1em]">Ten Days</em></>}>
      <p className="text-center text-[19px] text-[hsl(35,30%,68%)] leading-relaxed font-serif max-w-[640px] mx-auto mb-12">
      ...
      <div className="space-y-0.5">
        {[
          ...
          <div key={phase.phase} className="bg-[hsl(105,30%,12%)] border border-[hsla(45,70%,49%,0.12)] p-8 md:p-10 flex flex-col md:flex-row gap-5 md:gap-10">
            <div className="md:w-[140px] flex-shrink-0">
              <p className="font-sans text-[9px] tracking-[3px] uppercase text-[hsl(45,70%,49%)]">{phase.phase}</p>
              <p className="font-sans text-[12px] text-[hsl(40,30%,90%)] mt-1">{phase.title}</p>
            </div>
            <p className="text-[16px] text-[hsl(35,30%,68%)] leading-relaxed font-serif flex-1">{phase.desc}</p>
          </div>
        ))}
      </div>
    </SanctuarySection>
    ...
    <section className="px-6 md:px-12 py-16 md:py-20 max-w-[1160px] mx-auto" id="ceremonies">
      <div className="flex items-baseline gap-6 mb-12 pb-6 border-b border-[hsla(45,70%,49%,0.15)]">
        <span className="font-sans text-[8px] tracking-[4px] uppercase text-[hsl(45,70%,49%)]">Week One</span>
        <h2 className="font-sans text-[clamp(18px,2.5vw,28px)] font-extralight text-[hsl(40,30%,90%)]">
          March 18–23 · <em className="font-serif italic text-[hsl(45,70%,55%)]">The Core Ceremonies</em>
        </h2>
      </div>
    ...
    <section className="px-6 md:px-12 py-16 md:py-20 max-w-[1160px] mx-auto">
      <div className="flex items-baseline gap-6 mb-12 pb-6 border-b border-[hsla(45,70%,49%,0.15)]">
        <span className="font-sans text-[8px] tracking-[4px] uppercase text-[hsl(45,70%,49%)]">Week Two</span>
        <h2 className="font-sans text-[clamp(18px,2.5vw,28px)] font-extralight text-[hsl(40,30%,90%)]">
          March 25–29 · <em className="font-serif italic text-[hsl(45,70%,55%)]">Extended Offerings</em>
        </h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0.5">
        {weekTwo.map((card) => (
      ...
      <div className="max-w-[860px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0.5 mb-12 text-left">
          {[
            ...
            <div key={item.title} className="bg-[hsl(105,30%,12%)] border border-[hsla(45,70%,49%,0.12)] p-7">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-0.5 mt-12">
        {[
          ...
          <div key={step.num} className="bg-[hsl(105,30%,12%)] border border-[hsla(45,70%,49%,0.12)] p-8">
            <p className="font-sans text-2xl font-extralight text-[hsl(45,70%,49%)] mb-3">{step.num}</p>
